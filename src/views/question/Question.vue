      ...
      >
        ...
        >
          ...
          >
            ...
            <small style="margin-left:20px">问题</small>
...
export default {
    ...
    methods: {
        fetchData() {
            this.posts = null;
            this.loading = true;
            this.url = `/quests/list?pageNum=${this.currentPage}`;
            userService.questlist(this.url).then((response) => {
                this.loading = false;
                this.posts = response.data.data.quests;
                // console.log(response);
                console.log(response.data.data.quests);
        ...
        signup(id) {
            this.$router.push({ path: `/question/${id}` }); // 跳转的详情页面拼接id
        },
    },

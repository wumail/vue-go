    ...
    >
      ...
      >
        ...
        >
          ...
          <small style="margin-left:20px">{{code.id}}</small>
        </p>
        <p class="mb-0 text-desc">
          作者：{{code.user_name}} | 更新时间：{{code.updated_at}} | 分类：{{code.category}}
        </p>
        <hr
          ...
          size=1
        >
        <p style="margin:20px;min-height:200px;">
        ...
        <div
          ...
          font-size:18px"
          v-if="code.user_name == userInfo.name"
        >
          <b-button
            @click="updatep"
            variant="light"
      ...
      >
        ...
        >
          <b-media tag="li">
            ...
            <h5 class="mt-0 mb-1">{{code.user_name}}</h5>
            <span class="text-desc">发表于{{code.created_at}}</span>
          </b-media>
        </ul>
      </div>
    </div>
...
export default {
    ...
    methods: {
        fetchData() {
            ...
            userService.showcode(this.url).then((response) => {
        ...
        deletep() {
            this.url = `/codes/${this.id}`;
            userService.deletecode(this.url).then((response) => {
                console.log(response);
